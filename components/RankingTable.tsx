...
import type { Bias, SymbolResult } from "@/lib/types";
import { cn } from "@/lib/utils";

function TrendBadge({ trend }: { trend: Bias | null }) {
...
export default function RankingTable({ symbols }: { symbols: SymbolResult[] }) {
  const ranked = [...symbols].sort((a, b) => {
    const da = a.nearestBullishOrderBlock?.distancePct ?? Infinity;
    const db = b.nearestBullishOrderBlock?.distancePct ?? Infinity;
    return da - db;
  });
    ...
    <div className="overflow-x-auto rounded-xl border border-base-700 bg-base-850">
      <table className="w-full min-w-[640px] text-sm">
        ...
        <tbody>
          ...
          {ranked.map((s, i) => {
            const ob = s.nearestBullishOrderBlock;
            return (
              <tr key={s.ticker} className="border-b border-base-800 last:border-0">
                <td className="px-4 py-3 tabular text-[var(--text-muted)]">{i + 1}</td>
                <td className="px-4 py-3 font-medium">{s.ticker}</td>
                <td className="px-4 py-3 tabular">{s.lastPrice != null ? s.lastPrice.toFixed(3) : "—"}</td>
                <td className="px-4 py-3">
                  <TrendBadge trend={s.swingTrend} />
                </td>
                <td className="px-4 py-3">
                  <TrendBadge trend={s.internalTrend} />
                </td>
